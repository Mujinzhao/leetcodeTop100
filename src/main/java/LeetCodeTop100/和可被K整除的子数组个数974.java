package LeetCodeTop100;

/**
 * @ClassName 和可被K整除的子数组个数974
 * @Author zhangxinkun
 * @Date 2020/2/10  9:44 PM
 * @Version 1.0
 */

/**
 * 给定一个整数数组 A，返回其中元素之和可被 K 整除的（连续、非空）子数组的数目。
 *
 *  
 *
 * 示例：
 *
 * 输入：A = [4,5,0,-2,-3,1], K = 5
 * 输出：7
 * 解释：
 * 有 7 个子数组满足其元素之和可被 K = 5 整除：
 * [4, 5, 0, -2, -3, 1], [5], [5, 0], [5, 0, -2, -3], [0], [0, -2, -3], [-2, -3]
 *  
 *
 * 提示：
 *
 * 1 <= A.length <= 30000
 * -10000 <= A[i] <= 10000
 * 2 <= K <= 10000
 *
 * 解析：依然用前缀和数组进行求解，前缀和数组中同余的个数，然后求组合数，就是解。
 * 举一个例子，给定数组为 A = [4,5,0,-2,-3,1]。K=5；那么 前缀和 P = [0,4,9,9,7,4,5]，
 * ​
 *  =2（P[0]P[0]、P[6]P[6]），这表示一共有 \binom{2}{2} = 1(
 * 2
 * 2
 * ​
 *  )=1 的连续子数组的和能被 KK 整除，也就是 A[0:6] = [4, 5, 0, -2, -3, 1]A[0:6]=[4,5,0,−2,−3,1]。
 *
 * 对于 C_4 = 4C
 * 4
 * ​
 *  =4（P[1]P[1]、P[2]P[2]、P[3]P[3]、P[5]P[5]），这表示一共有 \binom{4}{2} = 6(
 * 2
 * 4
 * ​
 *  )=6 个连续子数组的和能被 KK 整除，分别是 A[1:2]A[1:2]、A[1:3]A[1:3]、A[1:5]A[1:5]、A[2:3]A[2:3]、A[2:5]A[2:5]、A[3:5]A[3:5]。
 */
public class 和可被K整除的子数组个数974 {
    public int subarraysDivByK(int[] A, int K) {
        int N = A.length;
        int[] P = new int[N+1];
        for (int i = 0; i < N; ++i) {
            P[i+1] = P[i] + A[i];
        }

        int[] count = new int[K];
        for (int x: P) {
            //因为这里有负数，所以需要先取模后再加K，然后再取模。
            count[(x % K + K)%K]++;
        }

        int ans = 0;
        for (int v: count) {
            ans += v * (v - 1) / 2;
        }
        return ans;
    }
}
